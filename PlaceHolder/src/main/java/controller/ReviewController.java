...
public class ReviewController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			// 1. User의 리뷰 작성하기 기능
			// 0. 현우 수정 리뷰 리스트 전체 조회
			if (cmd.equals("/selectAll.review")) {

				List<ReviewDTO> list = vdao.selectAll();

				request.setAttribute("reviewList", list);
				request.getRequestDispatcher("views/article/reviewList.jsp").forward(request, response);

				// 1. User의 리뷰 작성하기 기능
			} else if (cmd.equals("/write.review")) {
			...
			// 3. User의 리뷰 수정하기 기능
				...
				ReviewDTO reviewDto = new ReviewDTO(reviewId, null, null, null, reviewContent, reviewScore, null);
			...
			} else if (cmd.equals("/showReview.review")) {

				List<ReviewDTO> dto = vdao.showReviewToChange(request.getParameter("reviewId"));

				request.setAttribute("myReview", dto);
