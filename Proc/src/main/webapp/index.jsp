...
<div class="container">
		...
		<div class="container2">
			...
			<div class="board_fb_content">
				...
				<c:forEach var="board_dto" items="${fb_list}">
				<div class="board_fb_user_write">
						<div class="board_fb_user_num">${board_dto.fb_seq }</div>
						<div class="board_fb_user_title"><a href="/fb_detail.board?fb_seq=${board_dto.fb_seq }" style="text-decoration:none; color:black;">${board_dto.fb_title }</a></div>
						<div class="board_fb_user_writer">${board_dto.fb_mem_id }</div>
						<div class="board_fb_user_write_date">${board_dto.detailDate }</div>
						<div class="board_fb_user_view_count">${board_dto.fb_view_count }</div>
				</div>
				</c:forEach>
				<br>
				...
				<form action="/fb_search.board?cpage=1" id="frmSearch"> // 액션엔 뒤에 파라미터 못붙임 input타입 히든시켜서 네임으로 보내야됨
				<div class="board_fb_search_area">
					<select id="searchOption" name="searchOption">
						<option name="searchTitle" value="fb_title">제목</option>
						<option name="searchId" value="fb_mem_id">아이디</option>
					</select> <input type=text placeholder="검색어를 입력하세요" name="searchText" class="board_fb_search">
					<button type=submit class="board_fb_search_btn">검색
				</div>
				</form>
			</div>
		</div>
